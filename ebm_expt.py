...
def main(args):
	...
	ckpt = "./results/refuge_256-trg2src/00001-ls35-llr1.0000-lr0.0010-embed64-nembed512-chm8-beta0.5_0.999/ebm_010000.pt"
	latent_ema.load_state_dict(torch.load(ckpt))
	
	source_latent = torch.randn((8, 128, 64, 64)).to(device)

	source_latent_q = langvin_sampler(latent_ema, source_latent.clone().detach(),
										  langevin_steps=5000, lr=args.langevin_lr)

	source_image_t = ae.dec(source_latent_q)

	utils.save_image(source_image_t, 'source_image_t.png', normalize=True, range=(-1, 1))
