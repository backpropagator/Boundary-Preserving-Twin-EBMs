...
def random_apply_affine(img, p, G=None, antialiasing_kernel=SYM6):
    ...
    # img_2x = filter2D(img_pad,  kernel_flip.unsqueeze(0), normalized=True)
    # img_2x = F.conv2d(img_pad, weight=kernel_flip.view(1, 1, len_k, len_k).repeat(3, 1, 1, 1), groups=3, padding=1)
    grid = make_grid(
        img_2x.shape,
        -2 * p_ux1 / w_o - 1,
        2 * (w_p - p_ux1) / w_o - 1,
    ...
    # img_down = filter2D(img_affine, kernel.unsqueeze(0), normalized=True)
    # img_down = F.conv2d(img_affine, weight=kernel.view(1, 1, len_k, len_k).repeat(3, 1, 1, 1), groups=3, padding=1)
    end_y = -pad_y2 - 1
    if end_y == 0:
